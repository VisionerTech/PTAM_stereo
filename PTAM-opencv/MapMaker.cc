...
bool MapMaker::InitFromStereo(KeyFrame &kF,
			      ...
			      SE3<> &se3TrackerPose)
{
	//scale of the two keyframe, 0.1, but why every run makes a different size of the map
  ...
  while(!mbBundleConverged_Full)
    {
	  ...
	  count++;
	  cout<<"loop in bundleAdujust: "<<count<<endl;
	  if(count>STEREO_INIT_BA_TS)
	  {
		cout<<"infin bundle adjust, reset !"<<count<<endl;
		return false;
	  }
    }
...
SE3<> MapMaker::CalcPlaneAligner()
{
  ...
  for(int i=0; i<nRansacs; i++)
	...
	nC = rand()%nPoints;
	  ...
	  //cout<<"A:"<<mMap.vpPoints[nA]->v3WorldPos<<endl;
	  //cout<<"B:"<<mMap.vpPoints[nB]->v3WorldPos<<endl;
	  //cout<<"C:"<<mMap.vpPoints[nC]->v3WorldPos<<endl;

	  //cout<<"mean:"<<v3Mean<<endl;
  ...
  //udv = svd(m3Cov) last column of u
