...
ATANCamera::ATANCamera(std::string sName, float p1, float p2, float p3, float p4, float p5)
{
	msName = sName;
	Vector<NUMTRACKERCAMPARAMETERS> parameter_vecot = makeVector(p1,p2,p3,p4,p5);
	//GV2.Register(mgvvCameraParams, sName+".Parameters", mvDefaultParams, HIDDEN | FATAL_IF_NOT_DEFINED);
	mgvvCameraParams = parameter_vecot;
	mvImageSize[0] = 640.0;
	mvImageSize[1] = 480.0;
	RefreshParams();
}
