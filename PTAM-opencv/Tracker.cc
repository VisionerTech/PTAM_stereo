...
//void swap_image(CVD::Image<CVD::byte> &imFrame_L, CVD::Image<CVD::byte> &imFrame_R)
//{
//	CVD::Image<CVD::byte> temp = imFrame_L;
//	temp.make_unique();
//	imFrame_L = imFrame_R;
//	imFrame_R = temp;
//};
...
//try to init from stereo camera set.
void Tracker::TrackFrame(CVD::Image<CVD::byte> &imFrame_L, CVD::Image<CVD::byte> &imFrame_R, bool bDraw)
{
	//switch left and right camera 
	//if(mSwitchCamera)
	//{
	//	swap_image(imFrame_L, imFrame_R);
	//}
  ...
  if(mMap.IsGood())
      ...
      if(mnLostFrames < 3)  // .. but only if we're not lost!
	{
	  ...
	  if(mTrackingQuality == GOOD &&
	     mMapMaker.NeedNewKeyFrame(mCurrentKF) &&
	     mnFrame - mnLastKeyFrameDropped > LAST_KEYFRAME_DROP  &&
	     mMapMaker.QueueSize() < 3)
	    {
	      mMessageForUser << " Adding key-frame.";
	      AddNewKeyFrame();
    ...
    TrackForInitialMap_stereo(imFrame_L, imFrame_R); 
...
void Tracker::TrackForInitialMap_stereo(CVD::Image<CVD::byte> &imFrame_L, CVD::Image<CVD::byte> &imFrame_R)
{
	...
	if(mnInitialStage == TRAIL_TRACKING_NOT_STARTED) 
    {
		...
		if(mbUserPressedSpacebar)  // First spacebar = this is the first keyframe
		{
			...
			KeyFrame rightKF;
			rightKF.mMeasurements.clear();
			rightKF.MakeKeyFrame_Lite(imFrame_R);

			//making init feature points from left image top feature points only.
			...
			//match them with right images.
			int nGoodTrails = 0;
			MiniPatch BackwardsPatch;
			Level &lCurrentFrame = rightKF.aLevels[0];
			Level &lPreviousFrame = mCurrentKF.aLevels[0];
			...
			for(list<Trail>::iterator i = mlTrails.begin(); i!=mlTrails.end();)
			{
				...
				bool bFound = trail.mPatch.FindPatchRectify(irEnd, lCurrentFrame.im, 100, lCurrentFrame.vCorners);
				if(bFound)
				{
					...
					bFound = BackwardsPatch.FindPatchRectify(irBackWardsFound, lPreviousFrame.im, 100, lPreviousFrame.vCorners);
					if(bFound)
					{
			...
			m_msg = nGoodTrails;
			cout<<"#of goodTrails"<<nGoodTrails<<endl;
			if(nGoodTrails < STEREO_INIT_TS) // if most trails have been wiped out, no point continuing.
			{
				Reset();
				mMessageForUser << "not enough matches, build init map again please" << endl;
				cout<<"not enough matches, build init map again please"<<endl;
				return;
			}
			...
			if(!mMapMaker.InitFromStereo(mCurrentKF, rightKF,  vMatches, mse3CamFromWorld))// This will take some time!
			{
				Reset();
				mMessageForUser << "init from stereo false" << endl;
				cout<<"init from stereo false"<<endl;
				return;
			}
...
void Tracker::RenderGrid()
{
  ...
  Image<Vector<3> >	worldVertices(ImageRef(nTot,nTot));
  ...
  //is it memory save ?
  mGridRef.copy_from(imVertices);
  mGridVertices.copy_from(worldVertices);
