...
public:
	...
	std::string GetMessageForUser();
	  ...
	  enum TrackingQuality{BAD, DODGY, GOOD};
	  TrackingQuality mTrackingQuality;

	  //grid vertices projected in image buffer coordinate
	  CVD::Image<Vector<2> >  mGridRef;
	  //grid vertices in world coordinates
	  CVD::Image<Vector<3> >	mGridVertices;

		bool mbUserPressedSpacebar;
		//for some message out put to dll
		int m_msg;

		////boolean for switch left and right camera
		//bool mSwitchCamera;
...
protected:
  ...
  void TrackForInitialMap_stereo(CVD::Image<CVD::byte> &imFrame_L, CVD::Image<CVD::byte> &imFrame_R);//trying to build map from stereo.
